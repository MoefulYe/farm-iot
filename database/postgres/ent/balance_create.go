...
func (bc *BalanceCreate) SetWhen(t time.Time) *BalanceCreate {
	bc.mutation.SetWhen(t)
	return bc
}

// SetNillableWhen sets the "when" field if the given value is not nil.
func (bc *BalanceCreate) SetNillableWhen(t *time.Time) *BalanceCreate {
	if t != nil {
		bc.SetWhen(*t)
	}
	return bc
...
func (bc *BalanceCreate) defaults() {
	if _, ok := bc.mutation.When(); !ok {
		v := balance.DefaultWhen()
		bc.mutation.SetWhen(v)
	}
}
...
func (bc *BalanceCreate) createSpec() (*Balance, *sqlgraph.CreateSpec) {
	...
	if value, ok := bc.mutation.When(); ok {
